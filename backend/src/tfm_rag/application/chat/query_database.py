...
validated via `sql_safety.assert_read_only` before dispatching to the
connector.
"""
import base64
...
from tfm_rag.application.chat.sql_safety import assert_read_only
...
class _SourcesRepoLike(Protocol):
    # Narrow structural port; `SourceRepositoryPort` is assignable here.
...
async def query_database(
    ...
    *,
    ...
    # 1. Validate SQL.
    assert_read_only(inp.sql)

    # 2. Load source.
    try:
        row = await sources_repo.get_source_unscoped(inp.source_id)
