...
EMBEDDING_PROVIDER_CATALOG: dict[str, EmbeddingProviderDescriptor] = {
    ...
    "openai_compat": EmbeddingProviderDescriptor(
        ...
        requires_base_url_input=False,
        # Advisory only — these seed the UI dropdown. Any model the configured
        # endpoint actually serves is accepted (see EmbeddingSelection); the
        # model set is NOT hardcoded here. Examples: OpenAI text-embedding-3-*,
        # or Gemini's gemini-embedding-001 (dim 3072) via its OpenAI-compat API.
        default_models=(
