...
class Chatbot:
    ...
    `role_llm_selections` holds the per-role model overrides (evaluator /
    sql_generator / answer_generator); an unset role falls back to
    `llm_selection` via `RoleLLMSelections.resolve()`.
