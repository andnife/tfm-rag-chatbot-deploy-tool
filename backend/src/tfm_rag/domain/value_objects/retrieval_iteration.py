...
class TokenUsage:
    """Token usage telemetry for an LLM call."""
...
class RetrievalIteration:
    ...
    # plan 2026-07-01 — full SQL result table (markdown, truncated) so the
    # report/live trace can show what the query actually returned.
    ...
    def from_dict(cls, data: dict[str, Any]) -> "RetrievalIteration":
        return cls(
            ...
            result_preview=(
                str(data["result_preview"]) if data.get("result_preview") is not None else None
            ),
        )
