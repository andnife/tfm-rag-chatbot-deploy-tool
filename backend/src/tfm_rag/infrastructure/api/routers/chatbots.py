...
from tfm_rag.infrastructure.api.composition import build_answer_query_deps
...
class RoleLLMSelectionsIn(BaseModel):
    evaluator: LLMSelectionIn | None = None
    sql_generator: LLMSelectionIn | None = None
    answer_generator: LLMSelectionIn | None = None

    def to_vo(self) -> RoleLLMSelections:
        return RoleLLMSelections(
            evaluator=self.evaluator.to_vo() if self.evaluator else None,
            sql_generator=self.sql_generator.to_vo() if self.sql_generator else None,
            answer_generator=(
                self.answer_generator.to_vo() if self.answer_generator else None
            ),
        )
...
class PipelineConfigIn(BaseModel):
    ...
    max_self_correction_retries: int = Field(default=1, ge=0, le=3)
...
class WidgetConfigIn(BaseModel):
    ...
    welcome_message_named: str = Field(
        default="Hola {name}, ¿en qué puedo ayudarte?", min_length=1, max_length=500
    )
...
class CreateChatbotIn(BaseModel):
    ...
    role_llm_selections: RoleLLMSelectionsIn = Field(
        default_factory=RoleLLMSelectionsIn
    )
...
class UpdateChatbotIn(BaseModel):
    ...
    role_llm_selections: RoleLLMSelectionsIn | None = None
...
class ChatbotOut(BaseModel):
    ...
    role_llm_selections: dict[str, Any]
    ...
    def from_view(cls, v: ChatbotView) -> "ChatbotOut":
        return cls(
            ...
            role_llm_selections=v.role_llm_selections.to_dict(),
...
async def create_(
    ...
    try:
        view = await create_chatbot(
            ...
            role_llm_selections=body.role_llm_selections.to_vo(),
            kb_ids=body.kb_ids,
...
async def list_(
    ...
    views = await list_chatbots(
        chatbot_repo=ChatbotRepository(session, ctx), limit=limit, offset=offset
    )
...
async def get_(
    ...
    try:
        view = await get_chatbot(
            chatbot_repo=ChatbotRepository(session, ctx), chatbot_id=chatbot_id
        )
...
class WelcomeSuggestionsOut(BaseModel):
    welcome_message: str
    welcome_message_named: str


@router.post(
    "/{chatbot_id}/welcome-suggestions", response_model=WelcomeSuggestionsOut
)
async def welcome_suggestions_(
    chatbot_id: UUID,
    session: AsyncSession = Depends(get_session),  # noqa: B008
    ctx: RequestContext = Depends(get_current_context),  # noqa: B008
    settings: Settings = Depends(get_settings),  # noqa: B008
) -> WelcomeSuggestionsOut:
    """Draft (without persisting) the two welcome-message variants from the
    chatbot's knowledge. The panel pre-fills the editable fields with these
    and offers a "Regenerate" action."""
    try:
        msgs = await suggest_welcome_messages(
            chatbot_repo=ChatbotRepository(session, ctx),
            kb_repo=KnowledgeBaseRepository(session, ctx),
        ...
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
    return WelcomeSuggestionsOut(
        welcome_message=msgs.anonymous,
        welcome_message_named=msgs.named,
    )
...
async def patch_(
    ...
    try:
        view = await update_chatbot(
            ...
            role_llm_selections=(
                body.role_llm_selections.to_vo() if body.role_llm_selections else None
            ),
            kb_ids=body.kb_ids,
