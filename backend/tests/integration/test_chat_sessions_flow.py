...
async def test_get_session_isolation_between_tenants(
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        # extract_token prefers the persisted auth cookie (bob's, set by the
        # last register) over the Authorization header; clear it so the Bearer
        # token determines the acting user (else AliceBot lands in bob's tenant).
