...
async def test_kb_isolation_between_tenants(_clean_kb_tables: None) -> None:
    # One AsyncClient per user: register sets an httpOnly cookie and the auth
    # resolver prefers the cookie over the Bearer header, so a shared cookie jar
    # would make Alice's writes land under Bob's tenant (a test artifact, not a
    # real isolation failure). Separate clients keep the jars isolated.
    transport = ASGITransport(app=app)
    async with (
        AsyncClient(transport=transport, base_url="http://test") as alice,
        AsyncClient(transport=transport, base_url="http://test") as bob,
    ):
        alice_token, _ = await _register(alice, "alice-kb@example.com")
        bob_token, _ = await _register(bob, "bob-kb@example.com")
        alice_cred = await _ollama_credential_id(alice, alice_token)
