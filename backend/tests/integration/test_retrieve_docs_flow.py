...
async def test_search_on_other_tenants_kb_returns_404(_clean_state: None) -> None:
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        # The shared AsyncClient persists the auth cookie set by the last
        # register (bob's), and extract_token prefers the cookie over the
        # Authorization header — so without this, every request would act as
        # bob regardless of the Bearer token. Clear it so the explicit
        # Authorization header determines the acting user.
