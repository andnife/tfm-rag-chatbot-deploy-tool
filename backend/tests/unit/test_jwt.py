...
def test_jwt_carries_is_superadmin_when_set() -> None:
    token = encode_jwt(
        user_id=uuid4(), tenant_id=uuid4(), secret=SECRET,
        expires_hours=24, is_superadmin=True,
    )
    assert decode_jwt(token, SECRET)["sa"] is True


def test_jwt_defaults_is_superadmin_false() -> None:
    token = encode_jwt(
        user_id=uuid4(), tenant_id=uuid4(), secret=SECRET, expires_hours=24
    )
    assert decode_jwt(token, SECRET)["sa"] is False
