...
def test_default_registers_ollama_and_openai() -> None:
    disp = LLMDispatcher.default()
    assert disp.for_provider("ollama") is not None
    assert disp.for_provider("openai") is not None
    assert disp.for_provider("openai_compat") is not None
    with pytest.raises(UnsupportedProviderError):
        disp.for_provider("unknown_provider")
