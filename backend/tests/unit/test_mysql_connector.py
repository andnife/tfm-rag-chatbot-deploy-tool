...
async def test_test_connection_auth_failure_raises_database_connection_error(
    ...
    assert "access denied" not in msg.lower()
    assert "1045" not in msg
...
async def test_test_connection_network_failure_raises_database_connection_error(
    ...
    """T13: the raw OSError text must never reach the client."""
...
async def test_test_connection_network_failure_logs_raw_detail(
    monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture,
) -> None:
    """The sanitized detail must still be visible server-side (T13)."""
    _patch_connect(monkeypatch, raise_exc=OSError("Connection refused"))
    ...
    assert any("refused" in rec.message.lower() for rec in caplog.records)
...
async def test_introspect_schema_query_failure_raises_schema_introspection_error(
    ...
    msg = str(exc_info.value)
    assert msg == "schema introspection failed"
    assert "denied" not in msg.lower()
    assert "1142" not in msg
...
class _RunSelectConnection:
    def __init__(
        ...
        self._description = description
        self.closed = False
        self.last_cursor: _RunSelectCursor | None = None

    def cursor(self) -> _RunSelectCursor:
        self.last_cursor = _RunSelectCursor(self._rows, self._description)
...
async def test_run_select_starts_readonly_transaction(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The SELECT must run inside a read-only transaction so MySQL itself
    rejects any write, regardless of the connecting user's privileges.
    """
    conn = _RunSelectConnection([(1,)], [("n",)])
    _patch_connect_run_select(monkeypatch, fake_conn=conn)

    await MySQLConnector().run_select(_spec(), "SELECT 1 AS n", row_limit=10)

    assert conn.last_cursor is not None
    queries = conn.last_cursor.queries
    assert queries[0].upper().startswith("START TRANSACTION READ ONLY")
    # The actual SELECT still runs after the read-only transaction starts.
    assert any("SELECT" in q.upper() for q in queries[1:])
