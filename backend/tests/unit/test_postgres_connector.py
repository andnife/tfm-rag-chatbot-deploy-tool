...
class _FakeConnection:
    ...
    def __init__(self, rows_by_query: dict[str, list[dict[str, Any]]]) -> None:
        ...
        self.closed = False
        self.readonly_requested: bool | None = None

    def transaction(self, *, readonly: bool = False) -> _FakeTx:
        self.readonly_requested = readonly
        return _FakeTx()
...
async def test_test_connection_network_failure_raises_database_connection_error(
    ...
    """T13: the raw OSError text (which can include hostnames/ports) must
    never reach the client — only a generic, sanitized message.
    """
...
async def test_run_select_runs_inside_readonly_transaction(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The SELECT must run inside a read-only transaction so the DB itself
    rejects any write, even if the application-layer SQL filter is bypassed.
    """
    conn = _FakeConnection({})
    ...
    await PostgresConnector().run_select(_spec(), "SELECT 1 AS n", row_limit=10)
