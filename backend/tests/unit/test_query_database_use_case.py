...
def _source(
    *,
    ...
    """Build a domain Source entity with the attrs the use case reads."""
    return Source(
        id=source_id,
        kb_id=kb_id,
        type=type_,  # type: ignore[arg-type]
        payload=payload or _db_payload(),
        ingest_status="done",
        last_ingest_at=None,
        error=None,
    )
...
class _FakeSourcesRepo:
    def __init__(self, rows: dict[UUID, Source]) -> None:
        self._rows = rows
...
async def test_happy_path_dispatches_to_correct_driver_with_plain_password() -> None:
    ...
    repo = _FakeSourcesRepo({source_id: _source(
