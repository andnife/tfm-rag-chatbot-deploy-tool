...
def test_markdown_report_renders_generator_and_judge_models(tmp_path: Path) -> None:
    cases = [_scored("Q?", 0.7, 0.8, 0.6, 0.5)]
    report = _report(cases)
    paths = write_report(report, output_dir=tmp_path)

    md = paths.markdown_path.read_text(encoding="utf-8")
    assert "- Judge model: `llama3.1`" in md
    assert "- Generator model: `llama-3.3-70b`" in md
