...
# --- welcome_message_named (personalised greeting with {name}) ----------------


def test_welcome_message_named_default_is_non_empty_and_has_name_placeholder() -> None:
    cfg = WidgetConfig.default()
    assert cfg.welcome_message_named
    assert "{name}" in cfg.welcome_message_named


def test_welcome_message_named_round_trips() -> None:
    cfg = WidgetConfig.from_dict({"welcome_message_named": "Hola {name}!"})
    assert cfg.welcome_message_named == "Hola {name}!"
    assert cfg.to_dict()["welcome_message_named"] == "Hola {name}!"
    assert WidgetConfig.from_dict(cfg.to_dict()) == cfg


def test_welcome_message_named_missing_falls_back_to_default() -> None:
    cfg = WidgetConfig.from_dict({"theme": "dark"})
    assert cfg.welcome_message_named == WidgetConfig.default().welcome_message_named


def test_welcome_message_named_too_long_rejected() -> None:
    with pytest.raises(ValidationError):
        WidgetConfig.from_dict({"welcome_message_named": "x" * 1000})  # cap at 500
