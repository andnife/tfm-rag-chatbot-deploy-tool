...
# Bypass any inherited HTTP(S)_PROXY for local services. On WSL with a
# corporate proxy (WSL_PAC_URL injects HTTP_PROXY), httpx-based clients (Qdrant,
# Ollama) would otherwise route http://localhost:* through the proxy and fail
# with ConnectTimeout — while asyncpg (Postgres, raw TCP) ignores the proxy and
# works, which is the confusing "Postgres OK but Qdrant/Ollama 500" symptom.
# HTTP_PROXY itself is preserved so genuine external calls (Groq, OpenAI) still
# work; we only add the local hosts to the no-proxy list.
_LOCAL_NOPROXY='localhost,127.0.0.1,::1,postgres,qdrant,mysql_source,backend'
export NO_PROXY="${_LOCAL_NOPROXY}${NO_PROXY:+,$NO_PROXY}"
export no_proxy="${_LOCAL_NOPROXY}${no_proxy:+,$no_proxy}"
...
# --loop asyncio (not uvloop): RAGAS scoring uses nest_asyncio, which cannot
# patch uvloop ("Can't patch loop of type uvloop.Loop") and crashes eval runs.
exec uvicorn tfm_rag.infrastructure.api.app:app \
  --host 0.0.0.0 --port "$PORT" --loop asyncio $RELOAD "${EXTRA[@]}"
