#!/usr/bin/env bash
# Bootstrap the RAG chatbot platform on a fresh machine.
#
# What this does (idempotent — safe to re-run):
#   1. Verifies prerequisites: python3.12, docker, docker compose plugin,
#      node 20+ / npm (the latter two are soft checks — see step 7).
#   2. Creates the backend virtualenv at backend/.venv and installs deps
#      (including the `eval` extra, for RAGAS evaluation runs).
...
#   4. Pulls + starts the docker-compose services (postgres, qdrant,
#      mysql_source, ollama) and waits until they are healthy.
#   5. Applies alembic migrations against the running Postgres.
#   6. Runs the unit test suite as a smoke check.
#
#   7. Installs frontend dependencies (npm install in frontend/) — skipped
#      with a warning if Node 20+ / npm aren't available.
#
# What this does NOT do:
#   - Start uvicorn (use `scripts/run-backend.sh` for that, or run uvicorn
#     manually — see backend/README.md).
#   - Start the frontend dev server (use `scripts/run-frontend.sh`).
#   - Start both together (use `scripts/dev.sh`).
#
# Usage:
...
# Node + npm — needed for the frontend install (step 7). Not fatal here: a
# backend-only checkout can still run the rest of this script, but we want a
# clear, early message instead of a silent skip 90 seconds later.
NODE_OK=false
if command -v node >/dev/null 2>&1; then
  ...
  if (( node_ver >= 20 )); then
    ...
    warn "Node $(node -v) found but the frontend (next.config.js, Dockerfile) needs 20+. Frontend install will be skipped."
  fi
else
  warn "Node.js not found. Install Node 20+ (e.g. via nvm) to also set up the frontend, or skip it and run backend-only."
fi
if $NODE_OK && ! command -v npm >/dev/null 2>&1; then
  warn "npm not found alongside node. Frontend install will be skipped."
  NODE_OK=false
fi
...
pip install --quiet -e "$BACKEND_DIR[dev,eval]"
...
log "Starting docker-compose services (postgres, qdrant, mysql_source, ollama)…"
cd "$INFRA_DIR"
docker compose up -d postgres qdrant mysql_source ollama
...
while (( SECONDS < DEADLINE )); do
  ...
  my_state=$(docker compose ps --format json mysql_source 2>/dev/null | "$PYTHON_BIN" -c 'import sys,json; d=json.loads(sys.stdin.read() or "{}"); print(d.get("Health") or d.get("State",""))' || echo "")
  ...
  if [[ "$pg_state" == "healthy" && "$qd_state" == "healthy" && "$my_state" == "healthy" ]]; then
    ok "postgres: $pg_state | qdrant: $qd_state | mysql_source: $my_state | ollama: $ol_state"
...
if [[ -d "$FRONTEND_DIR" ]]; then
  if $NODE_OK; then
    ...
    warn "Skipping frontend install (see Node/npm check above). Install Node 20+ and run: cd frontend && npm install"
  fi
else
...
${GREEN}  Setup complete.${RESET}
  ...
  First user (once the backend is up):
      Open http://localhost:3000/register and sign up, or:
      curl -X POST http://localhost:8000/api/auth/register \\
        -H 'Content-Type: application/json' \\
        -d '{"email":"you@example.com","password":"yourpassword"}'

  • Integration tests (require Docker stack up — WARNING: TRUNCATEs your dev
    DB, see README "Tests"):
